...
***** DONE Web资源性能分析 [预计周三才有思博伦]
	  CLOSED: [2017-08-07 周一 14:09]
	  - State "DONE"       from "TODO"       [2017-08-07 周一 14:09]
**** TODO <2017-08-07 周一> [0%]
***** TODO 配合测试完成审计日志测试
***** TODO 清除TD上的BUG [0/4]
	  + [ ] BUG修复: 【远程应用】接入方式采用自创建账户方式，并启用同步，删除本地用户后服务器未同步删除用户
	  + [ ] 组合认证进行到一半后新用户登录，在线用户为组合认证用户
	  + [ ] SSL VPN DDos进程异常，封锁了自己的Mac
	  + [ ] Mac Safari打开控制台提示要提交证书，而实际没有开启证书认证
