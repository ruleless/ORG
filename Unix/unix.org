...
bool LockFreeQueue::EnQueue(const ElementT & ele) {
    ...
    char * cur_tail_flag_index = flags_array_ + curTailIndex;
    ...
    while (!__sync_bool_compare_and_swap(cur_tail_flag_index, 0, 1)) {
        ...
        cur_tail_flag_index = flags_array_ +  curTailIndex;
    ...
    int update_tail_index = (curTailIndex + 1) % mSize;

    // 如果已经被其他的线程更新过，则不需要更新；

    // 否则，更新为 (curTailIndex+1) % mSize;
